 ...
 */
async function getDataFromCSV(filePath, ignoreCompanies, maxRank, ignoreNoRank) {
    ...
    if (maxRank.trim() === '') { // Handle empty input
        maxRank = Infinity;
    }
...
async function startProductAnalysis(filePath, ignoreCompanies, maxRank, ignoreNoRank) {
    // Convert your existing script into a function that accepts parameters
    // Replace promptForCSVFilePath and promptForData with the passed parameters
    try {
        ...
        await getTokenAndMakeApiCall(maxRank, ignoreNoRank); // This gets the initial token
        calculateProfits();
        filterAndWriteToCSV();
        // Now perform the rest of your operations with the updated token
        // ...
        // Calculate profits
        // ...
